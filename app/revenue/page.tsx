...
export default function RevenueCenterPage() {
  ...
  return <main className="revenue-shell">
    ...
    <div className="os-nav"><button onClick={() => void executeQueue(1)} disabled={executing}>{executing ? "Worker 실행 중" : "다음 Job 실행"}</button><button onClick={() => void executeQueue(5)} disabled={executing}>최대 5건 실행</button><button onClick={() => void load()}>{loading ? "확인 중" : "새로고침"}</button></div>
