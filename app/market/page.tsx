...
export default function MarketPage() {
  ...
  return <main className="dashboard">
    <section className="hero market-hero"><div><p className="eyebrow">ENGINE 1 · MARKET INTELLIGENCE</p><h1>1. 시장정보·아이템 발굴</h1><p className="hero-description">수집·시계열·Feature Warehouse·다차원 기회점수·AI 판단·실매출 피드백을 하나의 데이터 자산으로 축적합니다.</p></div><div className="hero-actions"><Link className="button-link secondary-button" href="/market/finder">1-1. 아이템 발굴 워크벤치</Link><button disabled={working} onClick={() => runAction("/api/market/jobs/run", { limit: 20 }, "실행 가능한 수집 작업을 처리했습니다.", "market-collection-run")}>스케줄 실행</button><button disabled={working} onClick={() => runAction("/api/market/analyze", {}, "전체 시장 분석을 완료했습니다.")}>전체 분석</button><button className="secondary-button" disabled={working} onClick={() => runAction("/api/market/demo-seed", { keyword: keyword || "생활용품" }, "DEMO 데이터로 전체 파이프라인을 검증했습니다.")}>DEMO 검증</button></div></section>
    ...
    <section className="panel intelligence-panel" aria-labelledby="autonomous-item-recommendations-title">
      ...
      {intelligence.items?.length ? <div className="table-wrap"><table><thead><tr><th>형태</th><th>추천 후보</th><th>트렌드</th><th>점수</th><th>신뢰도</th><th>다음 검증</th></tr></thead><tbody>{intelligence.items.slice(0, 20).map((item) => <tr key={item.candidateId}><td>{item.form === "bundle" ? "묶음" : item.form === "set" ? "상품군" : "단품"}</td><td><Link href={`/admin/item-selection?keyword=${encodeURIComponent(item.concept)}`}><strong>{item.title}</strong></Link><small>{item.concept}</small></td><td>{item.trendState}</td><td>{Math.round(item.score)}</td><td>{Math.round(item.confidence)}%</td><td>{item.unresolved.map((value) => value.replaceAll("_", " ")).join(" · ")}</td></tr>)}</tbody></table></div> : <p className="empty-copy">유효 시장 신호가 확보되면 상품군 후보가 먼저 표시되고, 실제 관측 상품이 연결되면 단품·묶음 후보로 구체화됩니다.</p>}
    </section>
