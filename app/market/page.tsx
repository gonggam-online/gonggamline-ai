...
export default function MarketPage() {
  ...
  return <main className="dashboard">
    <section className="hero market-hero"><div><p className="eyebrow">MARKET INTELLIGENCE ENGINE · v8.0</p><h1>시장 데이터 수집·판단 운영체제</h1><p className="hero-description">수집·시계열·Feature Warehouse·다차원 기회점수·AI 판단·실매출 피드백을 하나의 데이터 자산으로 축적합니다.</p></div><div className="hero-actions"><Link className="button-link secondary-button" href="/">상품 운영</Link><Link className="button-link secondary-button" href="/discovery">AI 상품추천</Link><Link className="button-link secondary-button" href="/competition">경쟁력 분석</Link><button disabled={working} onClick={() => runAction("/api/market/jobs/run", { limit: 20 }, "실행 가능한 수집 작업을 처리했습니다.")}>스케줄 실행</button><button disabled={working} onClick={() => runAction("/api/market/analyze", {}, "전체 시장 분석을 완료했습니다.")}>전체 분석</button><button className="secondary-button" disabled={working} onClick={() => runAction("/api/market/demo-seed", { keyword: keyword || "생활용품" }, "DEMO 데이터로 전체 파이프라인을 검증했습니다.")}>DEMO 검증</button></div></section>
