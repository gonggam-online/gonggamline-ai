...
export type ExternalMarketProvider =
  "naver_api_hub" | "naver_shopping" | "youtube_data" | "dataforseo_naver";
...
function credentialsFromEnvironment(): ExternalProviderCredentials {
  return {
    naverClientId:
      process.env.NAVER_API_HUB_CLIENT_ID ?? process.env.NAVER_CLIENT_ID,
    naverClientSecret:
    ...
    dataForSeoMaxCostUsd: process.env.DATAFORSEO_MAX_COST_USD_PER_REQUEST
...
function boundedKeyword(keyword: string): string {
  ...
  if (normalized.length < 2 || normalized.length > 100)
    throw new Error("MARKET_KEYWORD_INVALID");
  return normalized;
}
...
export async function collectNaverApiHubTrends(
  keyword: string,
  options: Readonly<{
    credentials?: ExternalProviderCredentials;
    request?: Requester;
    now?: Date;
  }> = {},
  ...
  const categoryId =
    credentials.naverShoppingCategoryId?.trim() ??
    verifiedCategory?.categoryCode;
  if (categoryId && !/^\d{8,12}$/.test(categoryId))
    throw new Error("NAVER_SHOPPING_CATEGORY_INVALID");
  ...
  const searchBody = await jsonResponse(
    searchResponse,
  ...
  if (
    ...
    searchBody === null ||
    !Array.isArray((searchBody as { results?: unknown }).results)
  ) {
  ...
  if (categoryId) {
    ...
    const shoppingBody = await jsonResponse(
      shoppingResponse,
      "NAVER_API_HUB_SHOPPING_INSIGHT",
    );
    if (
      typeof shoppingBody !== "object" ||
      shoppingBody === null ||
      !Array.isArray((shoppingBody as { results?: unknown }).results)
    ) {
    ...
    const shoppingRecord =
      typeof shoppingResult === "object" && shoppingResult !== null
        ? (shoppingResult as Record<string, unknown>)
        : {};
...
function krwPriceFromText(value: unknown): number | null {
  if (typeof value !== "string") return null;
  const wonSuffix = value.match(
    /(?:₩|KRW\s*)?([0-9]{1,3}(?:,[0-9]{3})+|[0-9]{4,9})\s*원/i,
  );
...
function explicitSoldOutState(
  record: Record<string, unknown>,
  priceRecord: Record<string, unknown> | null,
): boolean | null {
  const explicit = [
    record.is_sold_out,
    record.sold_out,
  ...
  if (
    /(?:품절|일시품절|재고\s*없음|판매\s*중지|sold\s*out|out\s*of\s*stock|unavailable)/iu.test(
      searchable,
    )
  )
    return true;
  if (
    /(?:재고\s*있음|구매\s*가능|판매\s*중|배송\s*가능|in\s*stock|available)/iu.test(
      searchable,
    )
  )
    return false;
...
function ratingRecord(
  ...
  for (const value of [record.rating, record.product_rating]) {
...
function reviewCount(
  ...
  return (
    ...
    number(record.review_count) ??
    number(rating?.votes_count)
  );
}

function coupangDeliveryType(record: Record<string, unknown>): string | null {
  const searchable = [
    record.title,
    record.description,
    record.snippet,
    record.tags,
    record.delivery_info,
  ]
    .map((value) =>
    ...
    .normalize("NFC")
    .toLocaleLowerCase("ko-KR");
  if (/로켓그로스|판매자로켓/iu.test(searchable)) return "rocket-growth";
  if (/로켓배송|로켓와우|rocket/iu.test(searchable)) return "rocket";
  return null;
}
...
export async function collectDataForSeoCoupangPrices(
  ...
  const password = required(
    credentials.dataForSeoPassword,
    "DATAFORSEO_CREDENTIALS_MISSING",
  );
  ...
  const taskCost =
    typeof task === "object" && task !== null
      ? (number((task as Record<string, unknown>).cost) ?? 0)
      : 0;
  if (taskCost > maxCostUsd)
    throw new Error("DATAFORSEO_COST_CEILING_EXCEEDED");
  const result =
    typeof task === "object" &&
    task !== null &&
    Array.isArray((task as { result?: unknown }).result)
  ...
  const observations = nestedSerpItems(topLevelItems).flatMap(
    (record, index): MarketObservationInput[] => {
      ...
      if (
        ...
        currentPrice === null ||
        currentPrice <= 0 ||
        !/(^|\.)coupang\.com|쿠팡/i.test(sellerIdentity)
      )
        return [];
      if (currency && currency !== "KRW") return [];
      const rating = ratingRecord(record);
      const externalProductId =
        text(record.product_id, 200) ??
        text(record.data_docid, 200) ??
        url ??
        `coupang-serp-${index + 1}`;
      return [
        {
          ...
          snapshot: {
            rank:
              number(record.rank_absolute) ??
              number(record.rank_group) ??
              index + 1,
            isAd: null,
            price: currentPrice,
            ...
            rating:
              ...
              number(record.product_rating) ??
              number(rating?.value),
            reviewCount: reviewCount(record, rating),
            rocketType: coupangDeliveryType(record),
...
export async function collectYouTubeVideoSignals(
  ...
  const query = boundedKeyword(keyword);
  const key = required(
    (options.credentials ?? credentialsFromEnvironment()).youtubeApiKey,
  ...
  const searchBody = await jsonResponse(
    await request(searchUrl, { method: "GET", cache: "no-store" }),
    "YOUTUBE_SEARCH",
  );
  const searchItems =
    typeof searchBody === "object" &&
    searchBody !== null &&
    Array.isArray((searchBody as { items?: unknown }).items)
  ...
  if (videoIds.length) {
    ...
    const videoBody = await jsonResponse(
      await request(videoUrl, { method: "GET", cache: "no-store" }),
      "YOUTUBE_VIDEOS",
    );
    const videoItems =
      typeof videoBody === "object" &&
      videoBody !== null &&
      Array.isArray((videoBody as { items?: unknown }).items)
  ...
  if (channelIds.length) {
    ...
    const channelBody = await jsonResponse(
      await request(channelUrl, { method: "GET", cache: "no-store" }),
      "YOUTUBE_CHANNELS",
    );
    const channelItems =
      typeof channelBody === "object" &&
      channelBody !== null &&
      Array.isArray((channelBody as { items?: unknown }).items)
    ...
    for (const item of channelItems) {
      ...
      if (channelId && subscriberCount !== null)
        subscribersByChannel.set(channelId, subscriberCount);
      const channelSnippet =
        typeof record.snippet === "object" && record.snippet !== null
          ? (record.snippet as Record<string, unknown>)
          : {};
      const country = text(channelSnippet.country, 10);
      if (channelId && country)
        countryByChannel.set(channelId, country.toUpperCase());
    }
  }
  const signals = searchItems.flatMap(
    (item, index): MarketDiscoverySignal[] => {
      if (typeof item !== "object" || item === null) return [];
      const record = item as Record<string, unknown>;
      ...
      const statistics =
        typeof videoDetails.statistics === "object" &&
        videoDetails.statistics !== null
      ...
      const videoRecord =
        searchItems[index] && typeof searchItems[index] === "object"
          ? (searchItems[index] as Record<string, unknown>)
          : {};
      const detailsSnippet =
        typeof videoDetails.snippet === "object" &&
        videoDetails.snippet !== null
          ? (videoDetails.snippet as Record<string, unknown>)
          : {};
      const searchSnippet =
        typeof videoRecord.snippet === "object" && videoRecord.snippet !== null
          ? (videoRecord.snippet as Record<string, unknown>)
          : {};
      const channelId =
        text(detailsSnippet.channelId, 200) ??
        text(searchSnippet.channelId, 200);
      const channelTitle =
        text(detailsSnippet.channelTitle, 300) ??
        text(searchSnippet.channelTitle, 300);
      const description =
        text(detailsSnippet.description, 2_000) ??
        text(searchSnippet.description, 2_000);
      const tags = Array.isArray(detailsSnippet.tags)
        ? detailsSnippet.tags
      ...
      const contentDetails =
        typeof videoDetails.contentDetails === "object" &&
        videoDetails.contentDetails !== null
          ? (videoDetails.contentDetails as Record<string, unknown>)
          : {};
      const durationSeconds = isoDurationSeconds(contentDetails.duration);
      const thumbnails =
        typeof searchSnippet.thumbnails === "object" &&
        searchSnippet.thumbnails !== null
          ? (searchSnippet.thumbnails as Record<string, unknown>)
          : {};
      const mediumThumbnail =
        typeof thumbnails.medium === "object" && thumbnails.medium !== null
          ? (thumbnails.medium as Record<string, unknown>)
          : {};
      const thumbnailUrl = text(mediumThumbnail.url, 2_000);
      const ageDays = Math.max(
        1,
        (Date.parse(timestamp) - Date.parse(publishedAt)) / 86_400_000,
      );
      const popularityScore =
        views === null
          ...
          : Math.min(
              100,
              Math.round((Math.log10(Math.max(1, views)) / 7) * 10_000) / 100,
            );
      const contentVelocity =
        views === null
          ...
          : Math.min(
              100,
              Math.round(
                (Math.log10(Math.max(1, views / ageDays)) / 5) * 10_000,
              ) / 100,
            );
      return [
        discoverySignal({
          sourceId: "youtube-data-api",
          sourceKind: "short_video_public",
          query,
          externalProductId: videoId,
          title,
          ...
          subscriberCount: channelId
            ? (subscribersByChannel.get(channelId) ?? null)
            : null,
          channelCountry: channelId
            ? (countryByChannel.get(channelId) ?? null)
            : null,
          description,
          tags,
          durationSeconds,
          isShort: durationSeconds === null ? null : durationSeconds <= 180,
        }),
      ];
    },
  );
  const requestCount =
    1 + (videoIds.length ? 1 : 0) + (channelIds.length ? 1 : 0);
  return Object.freeze({
...
export async function collectDataForSeoNaverSignals(
  ...
  const basic = Buffer.from(`${login}:${password}`, "utf8").toString("base64");
  const response = await (options.request ?? fetch)(
    "https://api.dataforseo.com/v3/serp/naver/organic/live/advanced",
    {
      method: "POST",
      headers: {
        Accept: "application/json",
  ...
  const signals = items.flatMap((item, index): MarketDiscoverySignal[] => {
    ...
    return [
      discoverySignal({
        sourceId: "dataforseo-naver-serp",
        sourceKind: "paid_api",
        query,
        externalProductId,
        title,
        sourceUrl: url,
        observedAt: timestamp,
        rank: number(record.rank_absolute) ?? index + 1,
...
export async function collectExternalMarketProvider(
  ...
  if (provider === "naver_api_hub" || provider === "naver_shopping")
    return collectNaverApiHubTrends(keyword, options);
  if (provider === "youtube_data")
    return collectYouTubeVideoSignals(keyword, options);
