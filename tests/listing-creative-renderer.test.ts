...
test("selected-set approval binds evidence, category, policy, candidate, recipes and revision", async () => {
  const packet = await buildFixtureCreativeReviewPacket(
    planningInputFromListingContent(genericListingInput()),
  );
  const candidate = packet.candidates[0];
  const deployableCandidate = {
    ...candidate,
    artifacts: candidate.artifacts.map((artifact) => ({
      ...artifact,
      providerKind: "EXTERNAL_IMAGE_PROVIDER" as const,
      providerApprovalReference: "fixture:approved-provider-boundary",
      durableAssetReference: `https://assets.example.invalid/${artifact.byteDigest}.png`,
      deployability: "DEPLOYABLE" as const,
      review: { ...artifact.review, deployability: "PASS" as const },
    })),
  };
  const boundPacket = {
    ...packet,
    candidates: [deployableCandidate, packet.candidates[1]],
    selectedCandidateSetId: deployableCandidate.candidateSetId,
    contentApproval: {
      approved: true,
      approvalReference: "fixture:content-approval",
      boundArtifactDigests: deployableCandidate.artifacts.map(({ byteDigest }) => byteDigest),
      ...
      boundCandidateSetId: deployableCandidate.candidateSetId,
      boundTitleCandidateId: deployableCandidate.titleCandidateId,
      boundKeywordCandidateId: deployableCandidate.keywordCandidateId,
      boundRenderRecipeVersions: deployableCandidate.artifacts.map(({ renderRecipeVersion }) => renderRecipeVersion),
      boundRevisionId: packet.revisionId,
    },
  };

  assert.ok(mapApprovedCreativeCandidate(boundPacket));
  assert.equal(mapApprovedCreativeCandidate({ ...boundPacket, policyDigest: "changed-policy" }), null);
  assert.equal(mapApprovedCreativeCandidate({ ...boundPacket, categoryMetadataDigest: "changed-category" }), null);
  assert.equal(mapApprovedCreativeCandidate({ ...boundPacket, revisionId: "changed-revision" }), null);
});
