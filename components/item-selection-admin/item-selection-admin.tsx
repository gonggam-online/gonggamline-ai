...
export function ItemSelectionAdmin() {
  ...
  return (
    <DashboardLayout className="item-selection-admin">
      <DashboardHeader
        eyebrow="ENGINE 2 · SELECTION & PROFITABILITY"
        title="2. 상품선정·수익성"
            ...
            <div className="item-selection-admin__detail">
              ...
              {evaluations.length === 0 ? <DashboardEmptyState title="표시할 평가 결과가 없습니다" description="빈 결과이거나 선택한 판정 필터에 일치하는 항목이 없습니다." /> : <div className="item-selection-admin__evaluations">{evaluations.map((item, rank) => (
                <DashboardCard key={item.evaluationId} title={`기회 순위 #${rank + 1} · ${item.explainability?.provider.name ?? item.providerItemNumber}`} headingId={`evaluation-${item.evaluationId}`} actions={<span className={`item-selection-admin__badge item-selection-admin__badge--${item.verdict.toLowerCase()}`}>{VERDICT_LABELS[item.verdict]}</span>}>
                  <dl className="item-selection-admin__facts"><div><dt>기회 점수</dt><dd>{item.explainability?.score.availableDataScore === null || item.explainability?.score.availableDataScore === undefined ? "확인 필요" : `${item.explainability.score.availableDataScore}점`}</dd></div><div><dt>종합 평가 점수</dt><dd>{item.totalScoreUnits === null ? "판매 전 검증 필요" : `${(item.totalScoreUnits / 10_000).toFixed(1)}점`}</dd></div><div><dt>근거 범위</dt><dd>{item.coverageUnits / 10_000}%</dd></div><div><dt>기여이익</dt><dd>{item.normalizedProfitKrwMicros === null ? "판매가 입력 후 확인" : `${Math.round(Number(item.normalizedProfitKrwMicros) / 1_000_000).toLocaleString("ko-KR")}원`}</dd></div></dl>
                  {item.explainability ? <>
                    <h4>점수 근거</h4>
                    ...
                    {item.explainability.profitability.discoveryEstimate?.status === "ESTIMATED" ? <>
                      ...
                      <dl className="item-selection-admin__facts">
                        ...
                        <div><dt>가격 매칭 수준</dt><dd>{item.explainability.profitability.discoveryEstimate.marketSellingPrice?.matchType === "TITLE_MATCHED" ? "상품명 유사 매칭" : item.explainability.profitability.discoveryEstimate.marketSellingPrice?.matchType === "KEYWORD_COMPARABLE" ? "검색어 비교군" : "확인 필요"}</dd></div>
