...
|---|---|---|---|
| Naver Shopping Search API | 상품·가격·순위 메타데이터 | `NAVER_CLIENT_ID`, `NAVER_CLIENT_SECRET` | 공식 문서상 검색 API 하루 25,000회 한도, 유료 호출 없음 |
...
- 기본 수집은 Naver 30건, DataForSEO Naver depth 15, YouTube 10건으로
  제한한다. YouTube 결과는 `REFERENCE_ONLY/UNKNOWN` discovery signal로만
...
1. Vercel Production Secret 및 provider account budget 확인
2. `MARKET_EXTERNAL_PROVIDER`를 하나씩 선택하고
   `MARKET_EXTERNAL_PROVIDER_ENABLED=true` 설정
3. read-only smoke로 1개 keyword, 1회 호출 확인
4. 수집 결과를 Shadow/benchmark에서 검토
5. 운영 verdict 연결은 별도 Architecture/수동승인
